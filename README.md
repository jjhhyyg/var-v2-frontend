...
`src-tauri/resources/runtime/` and `src-tauri/resources/models/` are generated by `npm run desktop:build-worker` and must not be committed. The source repository only tracks `src-tauri/resources/placeholder.txt` to keep the directory structure.
